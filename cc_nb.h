...
namespace cc{
	...
	enum DynamicValue_Type{
		...
		DynamicValue_Type_String = 3,
		DynamicValue_Type_Bool = 4,
		DynamicValue_Type_IntArray = 5,
		DynamicValue_Type_FloatArray = 6
	};
	...
	public:
		...
		void checktype(DynamicValue_Type dtype) const{
			if (value.type != dtype){
				throw sformat("error type %d, except: %d\n", value.type, dtype);
			}
		}

		size_t floatarraySize() const{
			if (empty()) return 0;
			if (value.type == DynamicValue_Type_Float)
				return 1;

			checktype(DynamicValue_Type_FloatArray);
			return value.floatarrayv.size();
		}

		size_t intarraySize() const{
			if (empty()) return 0;
			if (value.type == DynamicValue_Type_Int)
				return 1;

			checktype(DynamicValue_Type_IntArray);
			return value.intarrayv.size();
		}
		...
		operator vector<int>() const{
			...
			checktype(DynamicValue_Type_IntArray);
			return value.intarrayv;
		}

		operator int() const{
			checktype(DynamicValue_Type_Int);
			return value.intV;
		}
		...
		operator float() const{
			checktype(DynamicValue_Type_Float);
			return value.floatV;
		}
...
#define Empty	DynamicValue()
	...
	struct ParamSpecMult{
		...
		string seril(){
			string part;
			part += sformat("lr_mult: %g\n", lr_mult);
			part += sformat("decay_mult: %g\n", decay_mult);
			return "param{\n" + part + "}";
		}
	};
	...
	namespace optimizer{
		...
		struct StochasticGradientDescent : public OOptimizer{
			...
			virtual string seril_sub_param(){
				string result;
				if (!momentum.empty()) result += sformat("momentum: %g\n", momentum.floatval());
	...
	namespace engine{
		...
		namespace caffe{
			...
			//
			std::shared_ptr<Net> buildNet(const vector<Tensor>& graphs, int phase=PhaseTest);
		};
	}
	...
	namespace train{
		...
		namespace caffe{
			...
			//    训练任务执行
			//
			void run(const Optimizer& optimizer, const std::function<StepEndCallbackFunctional>& stepEndCallback = nullptr);
		};
	};
};
